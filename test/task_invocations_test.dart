...
final _c = Task(
  noop,
  name: 'c',
  argsValidator: const ArgsCount.range(min: 2, max: 3),
);
final _d = Task(
  noop,
  name: 'd',
...
void main() {
  group('task invocations can be parsed correctly', () {
    ...
    test(
      ...
      () =>
          expect(parseInvocation(['a', ':b', ':c'], taskMap, const Options()), [
    ...
    test(
      ...
      () =>
          expect(parseInvocation(['a', ':X', 'b'], taskMap, const Options()), [
    ...
    test(
      ...
      () => expect(
        parseInvocation(['a', ':X', ':Y', 'b', ':Z'], taskMap, const Options()),
        [
  ...
  group('tasks can be sorted in execution order', () {
    ...
    test('with dependencies', () {
      expect(
        [_aw, _bw, _cw].sorted((a, b) => a.compareTo(b)),
        equals([_bw, _cw, _aw]),
      );
      expect(
        [_cw, _bw, _aw].sorted((a, b) => a.compareTo(b)),
        equals([_cw, _bw, _aw]),
      );
      expect(
        [_bw, _cw, _aw].sorted((a, b) => a.compareTo(b)),
        equals([_bw, _cw, _aw]),
      );
      expect(
        [_aw, _bw, _cw, _dw].sorted((a, b) => a.compareTo(b)),
        equals([_bw, _cw, _aw, _dw]),
      );
      expect(
        [_bw, _dw, _cw, _aw].sorted((a, b) => a.compareTo(b)),
        equals([_bw, _cw, _aw, _dw]),
      );
    });
    test('task phases are considered', () {
      ...
      final t4w = TaskWithDeps(
        Task((_) {}, name: '4', phase: TaskPhase.tearDown),
      );
      expect(
        [t1w, t2w, t3w, t4w].sorted((a, b) => a.compareTo(b)),
        equals([t1w, t2w, t3w, t4w]),
      );
      expect(
        [t2w, t1w, t4w, t3w].sorted((a, b) => a.compareTo(b)),
        equals([t1w, t2w, t3w, t4w]),
      );
      expect(
        [t4w, t2w, t1w, t3w].sorted((a, b) => a.compareTo(b)),
        equals([t1w, t2w, t3w, t4w]),
      );
      expect(
        [t4w, t3w, t2w, t1w].sorted((a, b) => a.compareTo(b)),
      ...
      expect(
        ...
        throwsA(
          isA<DartleException>().having(
            (e) => e.message,
            'expected message',
            equals("Invocation problem: Argument should follow a task: ':foo'"),
          ),
        ),
      );
    });
      ...
      expect(
        ...
        throwsA(
          isA<DartleException>().having(
            ...
            'expected message',
            equals(
              "Invocation problem: Invalid arguments for task 'd': [] - "
      ...
      expect(
        ...
        throwsA(
          isA<DartleException>().having(
            ...
            'expected message',
            equals(
              "Invocation problem: Invalid arguments for task 'c': [] - "
