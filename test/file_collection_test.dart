...
void main() {
  group('FilesCollection', () {
    ...
    test(
      'can be created for a single, non-empty directory (no recurse)',
      () async {
    ...
    test(
      'can be created for a single, non-empty directory (recurse)',
      () async {
    ...
    test(
      'can be created for root dir, with filter (recurse, exclude hidden)',
      () async {
        final files = dir(
          '.',
          fileExtensions: {'.txt'},
    ...
    test(
      'can be created for dir, with exclusions (recurse, include hidden)',
      () async {
        final files = dir(
          'A',
          exclusions: {'C', 'd.txt'},
        ...
        await _expectFileCollection(
          fs,
          files,
          dirs: const {'A', 'A/B', 'A/B/D', 'A/B/D/.git', 'A/B/D/E'},
          files: const {
            'A/B/D/.hide.txt',
            'A/B/D/E/e.txt',
            'A/B/D/E/.hide.txt',
    ...
    test(
      'can be created for root dir with extension filter including dot',
      () async {
        final files = dir('.', recurse: true, fileExtensions: {'.dart'});
        await _expectFileCollection(
          fs,
          files,
          files: const {'./dartle.dart'},
          dirs: const {
            '.',
            './dartle',
            './aa',
    ...
    test(
      'can be created for multiple directories with extension filter',
      () async {
        final files = dirs(
          const ['dartle', 'b', 'c', 'A'],
    ...
    test('file intersection', () async {
      expect(
        file('b/b.txt').intersection(files(const ['b/b.txt'])),
        equals({'b/b.txt'}),
      );
      ...
      expect(
        file('b/c/d.txt').intersection(dir('b', recurse: true)),
        equals({'b/c/d.txt'}),
      );
    });
    ...
    test('files intersection', () async {
      expect(
        files(const ['b/b.txt']).intersection(files(const ['b/b.txt'])),
        equals({'b/b.txt'}),
      );
      expect(
        files(const [
    ...
    test('dir intersection', () async {
      expect(
        dir('b').intersection(files(const ['b/b.txt'])),
      ...
      expect(
        dir('A/B/C').intersection(files(const ['A/B/C/c.txt'])),
      ...
      expect(
        dir('b/c/d', recurse: true).intersection(dir('b', recurse: false)),
        isEmpty,
      );
      expect(
        dir('b/c/d', recurse: true).intersection(dir('b', recurse: true)),
        equals(const {'b/c/d'}),
      );
    });
    ...
    test('intersection with extension filter', () async {
      ...
      expect(
        dir('b', fileExtensions: {'txt'}).intersection(dir('b')),
        equals({'b'}),
      );
      expect(
        dir('b', fileExtensions: {'txt'}).intersection(file('c.txt')),
        isEmpty,
      );
      expect(
        dir('b', fileExtensions: {'txt'}).intersection(file('b/c.txt')),
        equals({'b/c.txt'}),
      );
      expect(
        dir(
    ...
    test('output dir intersection with file extensions', () {
      final dartOutputs = dir(
        '.',
        exclusions: const {'build'},
        fileExtensions: const {'dart'},
      );
      final javaOutputs = dir(
        'test/java',
        fileExtensions: const {'.pom', '.jar'},
      );
    ...
    test('union containing dirs conflicts (hidden)', () async {
      ...
      await _expectFileCollection(
        ...
        dirs: const {'A/B/C', 'A/B/D', 'A/B/D/E'},
      );
    });
    ...
    test('union containing dirs conflicts (exclusion)', () async {
      ...
      await _expectFileCollection(
        ...
        dirs: const {'A/B', 'A/B/C', 'b', 'A/B/D/E'},
      );
    });
  }, timeout: Timeout(Duration(seconds: 5)));
}

Future _expectFileCollection(
  DartleTestFileSystem fs,
  FileCollection actual, {
...
class Entry {
  ...
  Entry.file(String name) : _name = name, _isFile = true, _bytes = const [];
