...
void main() {
  ...
  Future<ExecReadResult> runBuild(
    List<String> args, {
    Set<int> successExitCodes = const {0},
  }) async {
    return execRead(
      runDartExe(
        parallelTasksBuild,
        args: args,
        workingDirectory: 'test/test_builds/parallel_tasks',
      ),
  ...
  group('Parallel Tasks', () {
    test('can run with -p flag', () async {
      ...
      expect(
        proc.stdout[0],
        contains(
          'Executing 3 tasks out of a total of 6 tasks:'
          ' 3 tasks selected',
        ),
      );
    ...
    test('run in separate Isolates with -p flag', () async {
      ...
      expect(
        proc.stdout[0],
        contains(
          'Executing 4 tasks out of a total of 6 tasks:'
    ...
    test('loggers in separate Isolates with -p flag', () async {
      ...
      expect(
        proc.stdout[0],
        contains(
          'Executing 2 tasks out of a total of 6 tasks:'
          ' 2 tasks selected',
        ),
      );
      var tasksOutput = proc.stdout.skip(1).join();
    ...
    test('stackTraces are logged if log level is debug', () async {
      ...
      expect(
        tasksOutput,
        contains(
          RegExp(r'ERROR - Several errors have occurred', multiLine: true),
        ),
      );
      expect(
        tasksOutput,
