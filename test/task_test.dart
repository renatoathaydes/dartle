...
void main() {
      ...
      expect(
        ...
        throwsA(
          isA<DartleException>().having(
            ...
            'error message',
            equals(
              "Task with name 'g' does not exist "
              '(dependency path: [t -> g])',
            ),
          ),
        ),
  ...
  group('Tasks', () {
    ...
    test('task in different phases are not executed in parallel', () async {
      ...
      final t3 = TaskWithDeps(Task(noop, name: 't3', phase: TaskPhase.build), [
        t1,
      ]);
      final t4 = TaskWithDeps(Task(noop, name: 't4', phase: TaskPhase.build), [
        t1,
        t2,
        t3,
      ]);
      final t5 = TaskWithDeps(
        Task(noop, name: 't5', phase: TaskPhase.tearDown),
      );
      var tasksInOrder = await getInOrderOfExecution(
      ...
      expect(
        tasksInOrder[1].tasks.map((e) => e.task).toList(),
        equals([t2, t3]),
      );
  ...
  group('Task Verification', () {
    ...
    test('if a cleaning task affects other task, '
    ...
    test(
      'no error occurs if tasks have common ins/outs but dependency exists',
      () async {
        await verifyTaskInputsAndOutputsConsistency({
          'foo': TaskWithDeps(fooTask),
  ...
  group('Task Phase Verification', () {
    final setup1 = TaskWithDeps(
      Task(noop, name: 'setup1', phase: TaskPhase.setup),
    );
    final setup2 = TaskWithDeps(
      Task(noop, name: 'setup2', phase: TaskPhase.setup),
      [setup1],
    );
    final build1 = TaskWithDeps(
      Task(noop, name: 'build1', phase: TaskPhase.build),
    );
    final build2 = TaskWithDeps(
      Task(noop, name: 'build2', phase: TaskPhase.build),
      [setup1, setup2, build1],
    );
    final teardown1 = TaskWithDeps(
      Task(noop, name: 'teardown1', phase: TaskPhase.tearDown),
    );
    ...
    final phasesTaskMap = createTaskMap([
      setup1,
      setup2,
      build1,
      build2,
    ...
    test('tasks cannot depend on other tasks in a later phase than themselves', () {
      expect(
        () => verifyTaskPhasesConsistency({
          ...phasesTaskMap,
          'foo': TaskWithDeps(Task(noop, name: 'foo', phase: TaskPhase.setup), [
            build1,
          ]),
          'bar': TaskWithDeps(Task(noop, name: 'bar', phase: TaskPhase.build), [
            teardown1,
          ]),
        }),
        throwsA(
