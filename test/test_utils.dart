...
DartleTestFileSystem createTempFileSystem() {
  ...
  final tempDir = io.Directory(
    p.join(systemTemp.path, 'dartle-temp-${rand.nextDouble()}'),
  );
  tempDir.createSync();
...
Future<void> expectFileTree(
  String rootDir,
  Map<String, Object?> fileTree, {
  DartleTestFileSystem fs = const DartleTestFileSystem('.'),
  bool checkFileContents = true,
}) async {
  for (final entry in fileTree.entries) {
    final path = entry.key;
    if (entry.value == null) continue;
    final file = (path.endsWith('/') ? fs.directory : fs.file)(
    ...
    if (checkFileContents && file is io.File) {
      ...
      if (expectedContents is String) {
        ...
        throw Exception(
          'Cannot assert file contents using type: '
  ...
  await for (final entity in absRootDir.list(recursive: true)) {
    if (entity is io.File) {
      ...
      if (!fileTree.containsKey(path)) {
        fail(
          'Unexpected file in outputDir: $path. '
          'Actual tree: ${await _collectFileTree(fs, rootDir)}',
        );
      }
    }
